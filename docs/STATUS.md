...
1. Launch verification: complete authenticated hosted customer-account and admin journeys. Public route and API smoke checks pass; the local authenticated admin journey and the full local Playwright suite (27/27) now pass. Focused Vitest coverage is green; rerun and record the complete Vitest suite during release sign-off. Direct Vercel redeploys are temporarily blocked by the daily deployment quota.
