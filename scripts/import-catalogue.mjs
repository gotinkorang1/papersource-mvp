...
try {
  await sql.begin(async (tx) => {
    const [brand] = await tx`insert into brands (name, slug, active) values ('Imported Catalogue', 'imported-catalogue', true) on conflict (slug) do update set active = true returning id`;
    const categoryIds = new Map();
    for (const category of ["Stationery", "Books", "Others"]) {
      const slug = slugify(category);
      const [saved] = await tx`insert into categories (name, slug, active) values (${category}, ${slug}, true) on conflict (slug) do update set name = excluded.name, active = true returning id`;
    ...
    for (const [index, row] of rows.entries()) {
      ...
      const [product] = await tx`insert into products (name, slug, brand_id, category_id, product_type, status) values (${name}, ${slug}, ${brand.id}, ${category}, 'standard', 'active') on conflict (slug) do update set name = excluded.name, category_id = excluded.category_id, status = 'active', updated_at = now() returning id`;
      const [variant] = await tx`insert into product_variants (product_id, sku, name, unit_label, base_unit_price, currency, active) values (${product.id}, ${sku}, ${name}, 'each', ${price}, 'GHS', true) on conflict (sku) do update set product_id = excluded.product_id, name = excluded.name, base_unit_price = excluded.base_unit_price, active = true returning id`;
      await tx`insert into inventory (variant_id, on_hand, reserved, low_stock_threshold) values (${variant.id}, 0, 0, 0) on conflict (variant_id) do nothing`;
      await tx`delete from price_tiers where variant_id = ${variant.id}`;
      if (price === 0) await tx`insert into price_tiers (variant_id, minimum_quantity, unit_price, request_quote, currency, active) values (${variant.id}, 1, null, true, 'GHS', true)`;
    }
  });
