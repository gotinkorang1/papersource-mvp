...
export function PwaRegister() {
  ...
  if (iosInstall && !installEvent) return <div className="fixed inset-x-4 bottom-20 z-40 mx-auto max-w-md rounded-xl border border-border bg-card p-4 text-sm text-ink shadow-[0_18px_44px_rgba(16,42,67,0.2)] md:bottom-6"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border-2 border-ochre bg-ink text-xs font-bold tracking-[0.12em] text-cream" aria-hidden="true">PS</span><p className="min-w-0 flex-1"><span className="font-semibold">Add PaperSource to your Home Screen</span><span className="mt-0.5 block text-xs leading-5 text-slate">Tap Share, then choose <strong className="font-medium text-ink">Add to Home Screen</strong> for one-tap access.</span></p><button type="button" aria-label="Dismiss install instructions" onClick={() => { sessionStorage.setItem("papersource-install-dismissed", "1"); setDismissed(true); }} className="rounded-md px-2 py-1 text-xs text-slate hover:text-ink">×</button></div></div>;
