...
export function BusinessMenu() {
  ...
  return (
    <div className="relative" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
      <button type="button" className="min-h-11 text-sm text-graphite transition-colors hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ink" aria-expanded={open} aria-controls={menuId} aria-haspopup="menu" onClick={() => setOpen((value) => !value)} onKeyDown={(event) => event.key === "Escape" && setOpen(false)}>
        Business
      </button>
      {open ? (
        <div id={menuId} role="menu" className="absolute top-full left-0 z-30 min-w-52 origin-top rounded-lg border border-border bg-card p-4 pt-5 shadow-lg motion-safe:animate-in motion-safe:fade-in-0 motion-safe:zoom-in-95">
