...
export function ProductEngagement({ productId, productName, reviews }: { productId: string; productName: string; reviews: ProductReview[] }) {
  ...
  const share = async () => {
    ...
    else { await navigator.clipboard.writeText(url); setShared(true); window.setTimeout(() => setShared(false), 2400); }
  };
  return <section className="mt-14 border-t border-border pt-10" aria-labelledby="reviews-heading">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-medium uppercase tracking-[0.16em] text-slate">Customer experience</p><h2 id="reviews-heading" className="mt-2 text-2xl text-ink">Reviews & recommendations</h2><div className="mt-2 flex items-center gap-2 text-sm text-slate">{reviews.length ? <><Stars value={Math.round(average)} label={`${average.toFixed(1)} out of 5 stars`} /><span>{average.toFixed(1)} · {reviews.length} {reviews.length === 1 ? "review" : "reviews"}</span></> : <span>No reviews yet — be the first.</span>}</div></div><button type="button" onClick={share} className="min-h-10 rounded-md border border-ink px-4 py-2 text-sm font-medium text-ink transition-colors hover:bg-cream">{shared ? "Link copied" : "Share product"}</button></div>
