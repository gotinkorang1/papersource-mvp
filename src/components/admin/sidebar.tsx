...
export function AdminSidebar({ actor }: { actor: StaffActor }) {
  ...
  return (
    <>
      ...
      <button ref={accountTriggerRef} type="button" aria-expanded={accountOpen} aria-controls="admin-account-panel" onClick={() => setAccountOpen((open) => !open)} className="fixed right-4 top-[calc(0.75rem+env(safe-area-inset-top))] z-40 inline-flex min-h-11 items-center gap-2 rounded-full border border-border bg-card/95 px-3 text-sm font-semibold text-ink shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ink sm:right-6">
      ...
      <section id="admin-account-panel" aria-label="Staff account" aria-hidden={!accountOpen} inert={!accountOpen} className={`fixed inset-y-0 right-0 z-50 flex w-[min(20rem,calc(100vw-2rem))] flex-col border-l border-border bg-card p-6 text-ink shadow-2xl transition-transform duration-300 ease-out motion-reduce:transition-none ${accountOpen ? "translate-x-0" : "translate-x-full"}`}>
      ...
      <div className="flex min-h-16 items-center justify-between border-b border-sidebar-border px-4 py-3 sm:px-5 sm:py-4">
