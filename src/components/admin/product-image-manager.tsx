...
export function ProductImageManager({ productId, imageCount }: { productId: string; imageCount: number }) {
  ...
  const uploaded = items.filter((item) => item.status === "done" && item.publicId);
  ...
  return (
    <div className="mt-4 space-y-5">
      ...
      {uploaded.length > 0 ? <form action="/admin/products/mutate" method="post" className="grid gap-3 rounded-lg border border-paper-green/30 bg-paper-green/5 p-4"><input type="hidden" name="intent" value="add-image" /><input type="hidden" name="productId" value={productId} /><input type="hidden" name="cloudinaryPublicId" value={uploaded[0].publicId} /><label className="grid gap-1 text-sm font-medium text-ink">Alt text<input required value={alt} onChange={(event) => setAlt(event.target.value)} className={adminFieldClass} placeholder="Product image description" /></label><p className="text-xs text-slate">Add one uploaded image at a time so each image can have accurate accessible alt text.</p><SubmitProgressButton idleLabel="Add uploaded image" pendingLabel="Adding image…" className={paperButton()} /></form> : null}
