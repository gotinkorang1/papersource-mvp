    ...
    <div className="mt-4 space-y-5">
      <div className="rounded-lg border border-dashed border-border bg-cream/30 p-4 dark:bg-ink/30">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-2"><button type="button" className={paperButton({ variant: "secondary", className: "min-h-11" })} onClick={() => inputRef.current?.click()} disabled={!canAdd || uploadingAll}>Choose images</button>{items.some((item) => item.status === "ready" || item.status === "error") ? <button type="button" className={paperButton({ className: "min-h-11" })} onClick={() => void uploadAllPending()} disabled={uploadingAll}>{uploadingAll && uploadProgress ? `Uploading ${uploadProgress.done}/${uploadProgress.total}…` : "Upload all pending"}</button> : null}</div>
        </div>
        ...
        {items.some((item) => item.status === "done") ? <button type="button" className={paperButton({ className: "mt-4 min-h-11" })} onClick={() => void saveAllImages()} disabled={uploadingAll || savingAll || items.some((item) => item.status === "saving")} aria-busy={savingAll} aria-live="polite">{savingAll ? "Saving images…" : "Save all uploaded images"}</button> : null}
      </div>
