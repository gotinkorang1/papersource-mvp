...
import { orders, organizationMembers, quoteAccessTokens, quotes } from "@/lib/db/schema";
...
export async function listCustomerQuotes(profileId: string) {
  ...
  return db
    .select({
      ...
      accessToken: quoteAccessTokens.token,
    })
    .from(quotes)
    .leftJoin(quoteAccessTokens, eq(quoteAccessTokens.quoteId, quotes.id))
