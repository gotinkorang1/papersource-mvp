...
export default async function AdminCategoriesPage({ searchParams }: PageProps) {
  ...
  return (
    <main>
      ...
      <form className="mt-6 flex flex-wrap gap-2" method="get"><label className="sr-only" htmlFor="category-search">Search categories</label><input id="category-search" name="q" type="search" defaultValue={q} className={`${adminFieldClass} min-w-[16rem] flex-1`} placeholder="Search categories or slugs" /><select name="sort" defaultValue={sort} className={adminFieldClass}><option value="position">Sort: position</option><option value="name">Sort: name</option></select><SubmitProgressButton idleLabel="Search" pendingLabel="Searching…" className={paperButton({ variant: "secondary" })} />{q || sort !== "position" ? <Link href="/admin/categories" className="self-center text-sm text-slate underline">Clear</Link> : null}</form>
